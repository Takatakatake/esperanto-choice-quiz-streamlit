...
端末履歴の保存失敗は、失敗表示の維持、再読み込み後の1件だけの回復、容量回復で0件しか保存できない場合、明示消去の尊重、保存済み得点の再送防止を検査します。ブラウザーでも実クイズを完了させ、履歴書込みだけを失敗させた後の回復と消去を確認します。

累積更新は2スレッドで同じ名前の200点と225点の更新を重ね、225点が保持されること、別の名前・シートの更新を待たせないこと、通信例外後も次の更新が進むことを検査します。ランキングは不正な集計列とログ取得失敗を組み合わせ、取得不可・ログのみ・正常な0点の表示を区別します。
...
初期データの応答を待たせるテストでは、読み込み中の画面切替が無効で、完了後に学習記録へ移動できることも確認します。
