...
- 学習記録画面では、全体・単語・例文の累積点、単語の品詞・難易度別得点、例文のテーマ・小テーマ別得点を表示します。品詞は初期状態で折りたたみ、三角を開くと初級・中級・上級の内訳を確認できます。分野ごとの回数は保存済みクイズの回数です。
...
学習を始める前に、同じブラウザではこのアプリを1つのタブ・ウィンドウだけで開いてください。ホームや学習記録を表示した別タブ、同時に開いたPWAにも古い進行状態が残るため、同時に回答しなくてもタブを閉じる際の保存で進行状況が戻る場合があります。複数タブの進行状況と端末履歴を統合する機能はありません。
...
端末履歴の書込みに失敗した場合は、完了セッションに再試行待ちを記録し、再読み込み・通信復帰時に再試行します。「新しいクイズ」でも保存を再試行し、履歴が保存できるまで結果を保持します。後から保存に成功した場合は失敗表示を解除します。復旧状態の保存に失敗しても、復旧した履歴を容量整理で消さず、再試行待ちを維持します。記録済みの履歴を重複追加せず、明示的に消去した履歴は再作成しません。再試行待ちの記録も保存できなかった場合や、旧版ですでに失われた履歴を自動的に復元する機能ではありません。

通信タイムアウト時は間隔を空けて再試行し、結果画面や学習記録画面からの再試行にも対応します。未送信データを端末に保存できない場合、または端末履歴が再試行待ちの場合は完了結果を維持し、次のクイズによる上書きを止めます。ブラウザの保存データを手動で消すと、未送信の記録も失われます。
...
単語の難易度は保存済みの `group_id`（例：`noun:beginner_1:g2`）から集計します。初級・中級・上級は各品詞内での既存の出題区分をそのまま使い、未学習の区分は0点を表示します。少数語の品詞では出題グループが複数の難易度にまたがるため、その得点は「初級＋中級」などの内訳にまとめます。保存された点数はクイズ全体の点数なので、難易度ごとに推測で分配しません。グループ情報がない・不正・品詞と矛盾する記録は「レベル不明」に保持します。新しいシートや保存列の追加は不要です。
