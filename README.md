...
未送信の結果は、現在のクイズや直近履歴とは独立した永続送信待ちリスト（outbox）に、`save_id` ごとに保存します。再読み込み・次のクイズの開始・名前の切替後も、元の名前と同じIDで再送します。保存の確認応答が届くと、結果本体を小さな保存済み記録（receipt）に置き換え、別タブによる再登録を抑えます。旧outboxの配列は全件の移行を確認するまで保持します。
