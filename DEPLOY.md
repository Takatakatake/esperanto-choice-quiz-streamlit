...
同じサーバープロセス内では、同じスプレッドシート・集計シート・名前の累積更新を読み取りから確認まで順番に実行します。他の名前や別の集計シートの更新は待たせません。Google Sheetsは複数プロセス間の条件付き更新を提供しないため、同時書込みの完全な直列化は保証しません。保存IDでログ集計の重複を除外し、設定書込み後の曖昧な応答では旧変更の再送を止めます。既に開いている別タブのランキングには、更新操作まで以前の表示が残る場合があります。
...
- 集計表の列が不正: `UserStats` の `user` / `total_points` 列がそれぞれ1つあることを確認する。集計表を読めなくても `Scores` が正常なら保存ログから表示し、両方とも読めないときは取得失敗を返す。
