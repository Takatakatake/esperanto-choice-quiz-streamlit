...
Chromiumのブラウザー回帰テスト19件をGitHub Actionsの独立したジョブで実行します。確定結果はPRの `browser-regressions` チェックで確認できます。PC・スマートフォン表示、多言語、音声、個人進捗、アカウント別公開設定、保存・再読み込みに加え、保存確認用receiptの書込み失敗後の復元を検査します。実画面は接続済みブラウザーでも確認しました。書込みを伴う検証は全てメモリ内シートを使い、本番への仮得点追加はありません。Safari・Firefox・実機全機種の検証や、本番シートへの書込みテストは今回の確認範囲に含みません。
